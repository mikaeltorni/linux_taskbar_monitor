...
## Nuking Codex Installations

### Complete Removal Script

The `nuke_codex.sh` script provides a comprehensive way to remove all Codex-related installations from the system. This is useful when you need to start fresh or troubleshoot issues.

**Location:** `installation_scripts/nuke_codex.sh`

**What it removes:**
- Global and local NPM Codex packages (@openai/codex)
- All Codex wrapper scripts (codex, qwen, api, ca, cr, qa, qr, fla, fr, acc)
- Numbered Codex wrappers (codex1-10, qwen1-10, api1-10)
- Codex configuration and data directories (.codex, .codex-qwen, .codex-api)
- Agent-command-center tracker and related binaries
- LMStudio user service and configuration
- Codex autostart entries
- Backup and test files

**Usage:**
```bash
# For user-only removals (no sudo required for local items):
bash installation_scripts/nuke_codex.sh

# For complete removal including global NPM packages (requires sudo):
sudo bash installation_scripts/nuke_codex.sh
```

**Note:** The script preserves LMStudio binaries in `$HOME/.lmstudio/` as they may be used by other applications. To remove LMStudio as well, run:
```bash
rm -rf $HOME/.lmstudio
```

**Safety:** The script provides verification output showing what was removed and what remains (if anything).

### When to Use This Script

Use `nuke_codex.sh` when:
- Experiencing persistent issues with Codex agents
- Needing to reset to a clean state
- Before reinstalling Codex components
- Troubleshooting configuration conflicts
- Preparing for a fresh installation

The script is designed to be safe to run multiple times and will report what it finds and removes each time.