...
Disk space is configured with `--disk-space-perc-home-only`, so the panel shows the `/home` disk row as a colored used-space percentage and keeps disk throughput stats disabled. The disk patch renders live disk activity as a secondary colored percentage next to that primary usage value.
