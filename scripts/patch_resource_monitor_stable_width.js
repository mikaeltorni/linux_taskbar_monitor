// patch_resource_monitor_stable_width.js — Reserve fixed value-label widths so
// the panel does not shift as metric values change digit count (e.g. CPU
// 9% -> 10% -> 100%, disk activity 5% -> 100%, or GPU VRAM 8 -> 24 GB).
//
...
//
// Usage:
//   node scripts/patch_resource_monitor_stable_width.js <path-to-containers.js>

const fs = require("fs");

const containersPath = process.argv[2];
if (!containersPath) {
  console.error("Usage: patch_resource_monitor_stable_width.js <path-to-containers.js>");
  process.exit(1);
}
...
// Already patched: the secondary activity value width is reserved via a marker.
const DISK_MARKER =
...
// 3, so give VRAM its own tighter reserved width. Already patched guard:
...
if (!changed) {
  console.log("Stable widths already reserved");
}
